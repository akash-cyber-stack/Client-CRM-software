...
export default function Leads() {
  ...
  const load = () => {
    ...
    leadsApi.list(params).then((res) => setLeads(res.data.data)).finally(() => setLoading(false));
  };
  ...
  return (
    <div className="page-enter">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        <button className="btn-primary w-full sm:w-auto" onClick={() => setModalOpen(true)}>+ Add Lead</button>
      </div>
      ...
      {loading ? <LoadingSpinner /> : (
        <div className="card overflow-x-auto">
          ...
          {leads.length === 0 && <p className="text-center py-8 text-muted">No leads found</p>}
        </div>
      )}

      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add Manual Lead">
